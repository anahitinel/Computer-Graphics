...
#include <algorithm>
#include <vector>
#include "../part2/codes/vector.cpp"
#include <map>
#include <print>
#include <set>
...
public:
  ~TriangleMesh() {}
    ...
    void Tutte() {
        ...
        for (int i=0; i<indices.size(); i++) {
            vertex_to_neighbors[indices[i].vtxi].push_back(indices[i].vtxj);
            vertex_to_neighbors[indices[i].vtxi].push_back(indices[i].vtxk);

            vertex_to_neighbors[indices[i].vtxj].push_back(indices[i].vtxi);
            vertex_to_neighbors[indices[i].vtxj].push_back(indices[i].vtxk);

            vertex_to_neighbors[indices[i].vtxk].push_back(indices[i].vtxi);
            vertex_to_neighbors[indices[i].vtxk].push_back(indices[i].vtxj);
        };
        for (int i=0; i < indices.size(); i++) {
            for (auto it = vertex_to_neighbors_set[i].begin(); it != vertex_to_neighbors_set[i].end(); ++it) {
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            std::pair< int, int> edge1(indices[i].vtxi, indices[i].vtxj);
            ...
            std::pair< int, int> edge2(indices[i].vtxi, indices[i].vtxk);

            edge_to_triangles[edge2].push_back(i);


            int minKJ = std::min(indices[i].vtxk, indices[i].vtxj);
            int maxKJ = std::max(indices[i].vtxk, indices[i].vtxj);

            std::pair< int, int> edge3(indices[i].vtxk, indices[i].vtxj);
        ...
        for (auto it  = edge_to_triangles.begin(); it != edge_to_triangles.end(); ++it) {
            if (it -> second.size() ==1) {
                ...
                boundary_vtx[it->first] = true;
                boundary_vtx[it->second] = true;
            }
        }
        ...
        while (current_vtx != starting_vtx) {
            ...
            TriangleMesh parameterization;
            parameterization.indices = indices;
            parameterization.vertices = vertices;

            for (int i=0; i<boundary_chain.size(); i++) {
                double angle = i*2*M_PI / ((double)boundary_chain.size());
                double x = cos(angle);
                double y = sin(angle);
                parameterization.vertices[boundary_chain[i]] = Vector();
            }
            ...
            for (int iter=0; iter<1000; iter++) {
                for (int i=0; i<vertices.size(); i++) {
                    if (!boundary_vtx[i]) {
                        Vector avg(0, 0, 0);
                        int num_neighbors = vertex_to_neighbors[i].size();
                        for(int j=0; j<vertex_to_neighbors[i].size(); j++) {
                            avg = avg + parameterization.vertices[vertex_to_neighbors[i][j]]; //not sure...
                        }
                        avg = avg / num_neighbors;
                        new_vertices[i] = avg;
                    } else {
                        new_vertices[i] = parameterization.vertices[i];
                    }
                }
                parameterization.vertices = new_vertices;
            }

            parameterization.write_obj("tutte_result.obj");
